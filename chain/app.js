...
function bytopay(trig) {
    ...
    xhr.open(method, action + '?' + qstr, false);
    xhr.send();
    return false;
}


function bypost(trig) {
    var method = 'post';
    var action = trig.formAction || trig.name;
...
function dialog(trig, mode, pick, siz, title) {
    var stylec =
        siz == HALF ? ' uk-modal-half uk-animation-slide-bottom' :
            siz == SMALL ? ' uk-modal-small' :
                siz == LARGE ? ' uk-modal-large' :
                    ' uk-modal-full';
    ...
    if (tag == 'BUTTON') {
        ...
        trigc = (mode == OPEN && siz == FULL) ? ' button-open-full-trig' : ' button-trig';
    } else if (tag == 'A') {
        action = trig.href;
        method = 'get';
    ...
    if (mode != OPEN) {
        html += '<footer class="uk-modal-footer uk-text-center"><button id="okbtn" class="uk-button uk-button-primary" type="button" onclick="ok(this,' + mode + ',\'' + formid + '\',\'' + tag + '\',\'' + action + '\',\'' + method + '\');" disabled>确定</button></footer>'
    }
    html += '</section></div>';
    ...
    e.addEventListener('hidden', function (e) {
        ...
        var dlg = $('#dialog');
        if (dlg) {
            if (dlg.classList.contains('button-open-full-trig')) {
                location.reload(false);
            } else {
...
function ok(okbtn, mode, formid, tag, action, method) {
    var div = ancestor(okbtn, 'uk-modal');
...
function crop(trig, siz, title) {
    var wid,
        hei,
        sizg;
    var trigc = trig.tagName == 'BUTTON' ? ' button-open-full-trig' : ' anchor-trig';
    title = title || trig.innerHTML;
    ...
    html += '<button class="uk-button uk-button-primary" onclick="upload($(\'#imgbnd\'), \'' + action + '\');">确定</button>';
...
function checkToggle(trig) {
    var el = ancestor(trig, 'uk-card');
    if (!el) {
        el = ancestor(trig, 'tr');
    }
    if (!el) {
        el = ancestor(trig, 'uk-accordion-title');
    }
    if (trig.checked) {
