...
namespace Greatbone.Service
{
    ...
    public abstract class Service : Work, IHttpApplication<HttpContext>, ILoggerProvider, ILogger
    {
        ...
        protected Service(ServiceConfig cfg) : base(cfg)
        {
            ...
            string logfile = cfg.GetFilePath('$' + DateTime.Now.ToString("yyyyMM") + ".log");
            ...
            string cerfile = cfg.GetFilePath(ServiceUtility.CERT_PFX);
            if (File.Exists(cerfile) && cfg.certpass != null)
            {
                try
                {
                    X509Certificate2 cer = new X509Certificate2(cerfile, cfg.certpass, X509KeyStorageFlags.MachineKeySet | X509KeyStorageFlags.Exportable);
                    options.ConfigureHttpsDefaults(x => x.ServerCertificate = cer);
            ...
            logfac.AddProvider(this);

            server = new KestrelServer(Options.Create(options), ServiceUtility.TransportFactory, logfac);
            ...
            if (refs != null)
            {
                ...
                // distributed db operation azeas
                areas = new ConcurrentDictionary<string, DbArea>(factor, 16);
            }
        ...
        {
            XmlContent cnt = new XmlContent(false);
            Describe(cnt);
            return cnt.ToString();
        }
