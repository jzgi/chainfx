...
namespace Samp
{
    public abstract class TeamVarWork : Work
    {
        protected TeamVarWork(WorkConfig cfg) : base(cfg)
        {
        }
    }

    public class HublyTeamVarWork : TeamVarWork
    {
        public HublyTeamVarWork(WorkConfig cfg) : base(cfg)
        {
        }
        ...
        public async Task edit(WebContext wc)
        {
            ...
            if (wc.IsGet)
            {
                ...
                {
                    dc.Sql("SELECT ").collst(Team.Empty, 0).T(" FROM teams WHERE id = @1");
            ...
            else // POST
            {
                ...
                using (var dc = NewDbContext())
                {
                    dc.Sql("UPDATE teams")._SET_(Team.Empty, 0).T(" WHERE id = @1");
                    dc.Execute(p =>
                    {
                        o.Write(p, 0);
                        p.Set(orgid);
