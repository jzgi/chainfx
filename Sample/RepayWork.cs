    ...
    public class CtrRepayWork : RepayWork<CtrRepayVarWork>
    {
        ...
        public async Task reckon(WebContext wc)
        {
            ...
            else
            {
                ...
                {
                    dc.Execute(@"INSERT INTO repays (orgid, fro, till, orders, total, cash) SELECT orgid, @1, @2, COUNT(*), SUM(total), SUM(total * 0.994) FROM orders WHERE status = " + Ord.ENDED + " AND ended BETWEEN @1 AND @2 GROUP BY orgid", p => p.Set(fro).Set(till));
                }
                wc.GivePane(200);
            }
        }
