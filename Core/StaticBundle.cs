...
namespace Greatbone.Core
{
    public class StaticBundle
    {
        ...
        readonly Dictionary<string, Static> statics = new Dictionary<string, Static>(256);


        internal StaticBundle(string rootDir)
        {
            try
            {
                foreach (string path in Directory.GetFiles(rootDir))
                {
                    ...
                    {
                        string typ = null;
                        if (Types.TryGetValue(ext, out typ))
                        {
                            byte[] content = File.ReadAllBytes(path);
                            DateTime modified = File.GetLastWriteTime(path);
//                            statics.Add(new Static(Name: file.ToLower(), typ, content, modified));
                        }
                    }
                }
            }
