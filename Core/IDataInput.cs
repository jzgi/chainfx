...
namespace Greatbone.Core
{
    ...
    public interface IDataInput
    {
        ...
        bool Get<D>(string name, ref D v, ushort sel = 0) where D : IData, new();

        bool Get<D>(string name, ref D[] v, ushort sel = 0) where D : IData, new();

        bool Get<D>(string name, ref List<D> v, ushort sel = 0) where D : IData, new();

        D ToObject<D>(ushort sel = 0) where D : IData, new();

        D[] ToArray<D>(ushort sel = 0) where D : IData, new();

        List<D> ToList<D>(ushort sel = 0) where D : IData, new();
