...
function dialog(trig, mode, pick, siz, title) {
    var stylec = siz == 1 ? ' uk-modal-small' :
        siz == 2 ? ' uk-modal-half uk-animation-slide-bottom' :
            siz == 3 ? ' uk-modal-side uk-animation-slide-right' :
                siz == 4 ? ' uk-modal-large' : ' uk-modal-full';
    ...
    var html = '<div class="uk-modal ' + stylec + trigc + '" uk-modal="bg-close: false">';
    html += '<section class="uk-modal-dialog ">';
    ...
    html += '<main class="uk-modal-body uk-padding-remove"><iframe src="' + src + '" style="width: 100%; height: 100%; border: 0"></iframe></main>';
    if (mode != OPEN) {
        html += '<footer class="uk-modal-footer uk-text-center"><button id="okbtn" class="uk-button uk-button-default uk-border-rounded" type="button" onclick="ok(this,' + mode + ',\'' + formid + '\',\'' + tag + '\',\'' + action + '\',\'' + method + '\');" disabled>确定</button></footer>'
    }
    html += '</section></div>';
...
function crop(trig, ordinals, siz, title) {
    ...
    if (ordinals > 0) {
        html += '<select id="imgord" onchange="bind(document.getElementById(\'imgbind\'), \'' + action + '\', this.value, ' + wid + ', ' + hei + ')">';
        for (var i = 1; i <= ordinals; i++) {
            html += '<option value="' + i + '">' + i + '</option>';
        }
        html += '</select>';
    }
    html += '<button class="uk-button uk-button-default" onclick="document.getElementById(\'imginp\').click()">选择</button>';
    html += '<button class="uk-button uk-button-default" onclick="upload(document.getElementById(\'imgbind\'), \'' + action + '\', (document.getElementById(\'imgord\') ? document.getElementById(\'imgord\').value : 0));">上传</button>';
    html += '</nav>'; // control group
    html += '<button class="uk-modal-close-default" type="button" uk-close></button>';
    html += '</header>'; // header
    html += '<main id="imgbind" class="uk-modal-body uk-padding-remove">'; // body
    html += '<input type="file" id="imginp" style="display: none;" onchange="bind(this.parentNode, window.URL.createObjectURL(this.files[0]), (document.getElementById(\'imgord\') ? document.getElementById(\'imgord\').value : 0),' + wid + ',' + hei + ');">';
    html += '</main>';
    html += '</section>'; // dialog
    html += '</div>'; // modal
    ...
    var e = appendTo(document.body, html);
    UIkit.modal(e).show();
    bind(document.getElementById('imgbind'), action, (ordinals > 0 ? 1 : 0), wid, hei);
    e.addEventListener('hidden', function () {
        document.body.removeChild(e);
    }, false);
    return false;
