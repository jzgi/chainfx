...
function byprepay(trig) {
    ...
    xhr.onload = function (e) {
        ...
        window.top.WCPay(data); // call top windows's weixin pay bridge
        // close dialog
        var dlg = $('#dialog');
        if (dlg) UIkit.modal(dlg).hide();
    };
    // send request
...
function initform() {
    ...
    for (i = 0; i < fsets.length; i++) {
        var fset = fsets[i];
        var sum = 0;
        var hids = fset.querySelectorAll('input[type=hidden]');
        for (k = 0; k < hids.length; k++) {
            var hid = hids[k];
            sum += parseInt(hid.dataset.qty);
        }
        var num = fset.querySelector('input[type=number]');
        num.max = sum;
        num.value = sum;
    }
}
...
function dialog(trig, mode, pick, siz, title) {
    ...
    e.addEventListener('hidden', function () {
        ...
        if (dlg) {
            ...
            if (history.state) { // if currently on dialog
                history.back();
            }
        }
    }, false);
    ...
    window.addEventListener("popstate", function (e) {
        if (!e.state) { // if currently not on dialog
            var dlg = $('#dialog');
            if (dlg) UIkit.modal(dlg).hide();
        }
    });
...
function crop(trig, ordinals, siz, title) {
    ...
    if (ordinals > 0) {
        html += '<select id="imgord" onchange="bind($(\'#imgbnd\'), \'' + action + '\', this.value, ' + wid + ', ' + hei + ')">';
        for (var i = 1; i <= ordinals; i++) {
            html += '<option value="' + i + '">' + i + '</option>';
        }
        html += '</select>';
    }
    html += '<a class="uk-button uk-button-default" onclick="$(\'#imginp\').click()">选择</a>';
    html += '<button class="uk-button uk-button-default" onclick="upload($(\'#imgbnd\'), \'' + action + '\', ($(\'#imgord\') ? $(\'#imgord\').value : 0));">上传</button>';
    html += '</nav>'; // control group
    html += '<button class="uk-modal-close-default" type="button" uk-close></button>';
    html += '</header>'; // header
    html += '<main id="imgbnd" class="uk-modal-body uk-padding-remove">'; // body
    html += '<input type="file" id="imginp" style="display: none;" onchange="bind(this.parentNode, window.URL.createObjectURL(this.files[0]), ($(\'#imgord\') ? $(\'#imgord\').value : 0),' + wid + ',' + hei + ');">';
    html += '</main>';
    html += '</section>'; // dialog
    html += '</div>'; // modal
    ...
    var e = appendTo(document.body, html);
    UIkit.modal(e).show();
    bind($('#imgbnd'), action, (ordinals > 0 ? 1 : 0), wid, hei);
    e.addEventListener('hidden', function () {
        var dlg = $('#dialog');
        if (dlg) {
            document.body.removeChild(dlg);
