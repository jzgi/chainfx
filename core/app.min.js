...
function crop(trig, ordinals, siz, title) {
    ...
    if (ordinals > 0) {
        html += '<select id="imgordinal" onchange="bind(this.form.querySelector(\'.crop\'), \'' + action + '\', this.value, ' + wid + ', ' + hei + ')">';
    ...
    html += '<button class="uk-button uk-button-default uk-border-rounded" onclick="upload(this.form.querySelector(\'.crop\'), \'' + action + '\', this.form.ordinal.value);">上传</button>';
    html += '</nav>'; // control group
    html += '<button class="uk-modal-close-default" type="button" uk-close></button>';
    html += '</header>'; // header
    html += '<main class="uk-modal-body uk-padding-remove crop">'; // body
    html += '<input type="file" id="imginp" style="display: none;" onchange="bind( this.parentNode, window.URL.createObjectURL(this.files[0]), document.getElementById(\'imgordinal\') ? document.getElementById(\'imgordinal\').value : 0,' + wid + ',' + hei + ');">';
    html += '</main>';
    html += '</section>'; // dialog
    html += '</div>'; // modal
    ...
    var e = appendTo(document.body, html);
    UIkit.modal(e).show();
    bind(e.querySelector('.crop'), action, (ordinals > 0 ? 1 : 0), wid, hei);
    e.addEventListener('hidden', function () {
        document.body.removeChild(e);
    }, false);
    return false;
